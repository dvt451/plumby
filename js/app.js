(() => {
    ...
    (() => {
        ...
        if (document.querySelector(".select")) {
            const selectParent = document.querySelector(".select");
            const natSelect = document.querySelector("select");
            document.querySelector(".selected");
            const natOptions = document.querySelectorAll("option");
            ...
            for (let i = 0; i < natOptions.length; i++) {
                const natOption = natOptions[i];
                let option = document.createElement("div");
                option.className = "option";
                option.innerText = natOption.innerText;
        ...
        for (let index = 0; index < menu__lists.length; index++) {
            const menuListItems = menu__lists[index];
            for (let i = 0; i < menuItems.length; i++) {
                const item = menuItems[i];
                const li = document.createElement("li");
                li.classList.add("menu__item");
                let a;
                if (menuListItems.parentElement.classList.contains("burger__menu")) {
                    a = document.createElement("Button");
                    a.classList.add("menu__link");
                    let aText = document.createElement("span");
                    aText.innerText = item.text;
                    let aArrow = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="15" viewBox="0 0 14 15" fill="none"><path d="M7.49932 10.8155C7.22726 11.0875 6.77274 11.0875 6.49998 10.8155L0.206656 4.53826C-0.0688853 4.26272 -0.0688853 3.81587 0.206656 3.54103C0.482197 3.26549 0.929748 3.26549 1.20529 3.54103L6.99997 9.3196L12.794 3.54033C13.0702 3.26479 13.517 3.26479 13.7933 3.54033C14.0688 3.81587 14.0688 4.26272 13.7933 4.53757L7.49932 10.8155Z" fill="#060606"/></svg>';
                    let aIcon = document.createElement("i");
                    aIcon.innerHTML = aArrow;
                    a.append(aText, aIcon);
                } else {
                ...
                if (menuListItems.parentElement.classList.contains("footer__menu")) li.setAttribute("data-aos", "zoom-in-up");
                li.append(a);
                menuListItems.append(li);
                const submenuList = document.createElement("ul");
                submenuList.classList.add("menu__submenu-list");
                li.append(submenuList);
                for (let ind = 0; ind < item.submenu.length; ind++) {
                    const submenu_Item = item.submenu[ind];
                    const submenuItem = document.createElement("li");
                    submenuItem.classList.add("menu__submenu-item");
                    submenuList.append(submenuItem);
                    const submenuLink = document.createElement("a");
                    submenuLink.setAttribute("href", "#");
                    submenuLink.classList.add("menu__submenu-link");
                    submenuItem.append(submenuLink);
                    submenuLink.innerText = submenu_Item.text;
                }
            }
        }
        const headerMenuItem = document.querySelectorAll(".header__menu .menu__item");
        const hederSubmenuList = document.querySelectorAll(".menu__submenu-list");
        for (let i = 0; i < headerMenuItem.length; i++) {
            const item = headerMenuItem[i];
            item.addEventListener("mouseenter", (function(e) {
                hederSubmenuList[i].classList.add("_active");
            }));
            item.addEventListener("mouseleave", (function(e) {
                hederSubmenuList[i].classList.remove("_active");
            }));
        }
        let submenuButton = document.querySelectorAll(".burger__menu .menu__link");
        let burgerSubmenuList = document.querySelectorAll(".burger__menu .menu__submenu-list");
        const burger_menu__link = document.querySelectorAll(".burger-body .menu__link");
        for (let i = 0; i < submenuButton.length; i++) {
            const item = submenuButton[i];
            item.addEventListener("click", (function(e) {
                _slideToggle(burgerSubmenuList[i]);
                burger_menu__link[i].classList.toggle("_active");
            }));
        }
        aos.init({
            offset: 0,
